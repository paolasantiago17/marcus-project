      ...
      <div class="scroll" style="padding:0 24px;">
        <div style="display:flex; align-items:center; gap:16px; padding-bottom:24px; border-bottom:1px solid rgba(27,25,22,.10); margin-bottom:24px;">
          ...
          <div style="min-width:0;">
            ...
            <p style="margin:0; font-size:12.5px; font-weight:300; color:#8C8375;">${esc(p.email)}</p>
          </div>
        </div>
        ...
        <div style="display:flex; gap:10px; margin-bottom:28px;">
          <div style="flex:1; border-radius:16px; background:#15130F; padding:18px;">
            ...
            <p style="margin:0; font-size:10px; letter-spacing:.14em; text-transform:uppercase; color:#9A8F79;">Points</p>
          </div>
          <div style="flex:1; border-radius:16px; background:#F2ECE0; padding:18px;">
            ...
            <p style="margin:0; font-size:10px; letter-spacing:.14em; text-transform:uppercase; color:#8C8375;">Raffle entr${tier.prevEarned && Store.TIER_THRESHOLDS.indexOf(tier.prevEarned) + 1 === 1 ? 'y' : 'ies'}</p>
          </div>
          <div style="flex:1; border-radius:16px; background:#F2ECE0; padding:18px;">
            <p class="h-serif" style="font-size:26px; margin-bottom:6px;">${mine.length}/3</p>
            <p style="margin:0; font-size:10px; letter-spacing:.14em; text-transform:uppercase; color:#8C8375;">Submitted</p>
          </div>
        </div>
        ...
        ${mine.length ? `
          ...
          <div class="card" style="padding:16px; margin-bottom:28px;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center;">
              <span style="font-size:12.5px; font-weight:300; color:#8C8375;">Submitted ${new Date(mine[0].submittedAt).toLocaleDateString()}</span>
              <span style="font-size:11px; letter-spacing:.12em; text-transform:uppercase; color:#2E6B5C;">${accepted} accepted${inReview ? ` · ${inReview} in review` : ''}</span>
            </div>
          </div>` : `
          <div class="card" style="padding:18px 20px; margin-bottom:28px;">
            <p style="margin:0; font-size:13.5px; font-weight:300; color:#4A443A;">You haven't submitted your three photos yet.</p>
          </div>`}
        ...
        <div style="border-top:1px solid rgba(27,25,22,.10);">
          ...
          <div data-nav="#/notices" class="acct-row"><span>Notices</span>${unread ? `<span style="height:22px; min-width:22px; padding:0 7px; border-radius:11px; background:#C4543A; color:#FFFDF8; font-size:11px; display:flex; align-items:center; justify-content:center;">${unread}</span>` : `<span style="color:#8C8375;">&rarr;</span>`}</div>
          <div data-nav="#/feedback" class="acct-row"><span>Send feedback</span><span style="color:#8C8375;">&rarr;</span></div>
          <div data-nav="#/terms" class="acct-row"><span>Terms of Use</span><span style="font-size:12px; color:${p.termsVersion === Store.TERMS_VERSION ? '#8C8375' : '#C4543A'};">${p.termsVersion === Store.TERMS_VERSION ? 'Accepted' : p.termsVersion ? 'Updated' : 'Not accepted'} &rarr;</span></div>
        ...
        <p style="margin:24px 0 26px; font-size:11.5px; font-weight:300; line-height:1.7; color:#8C8375;">ArtUP Campus Canvas · Queen's University 2027 pilot · Build 1.0.4</p>
      </div>
    </div>
    <style>.acct-row{display:flex;justify-content:space-between;align-items:center;padding:16px 0;border-bottom:1px solid rgba(27,25,22,.07);font-size:14.5px;font-weight:300;cursor:pointer;}</style>`;
