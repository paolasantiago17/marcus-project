    ...
    <div class="screen">
      <div class="topbar" style="padding-bottom:20px;">
        ...
        <div style="display:flex; align-items:center; gap:10px;">
          <span style="font-size:10.5px; letter-spacing:.16em; text-transform:uppercase; color:#8C8375;">We read everything</span>
      ...
      <div class="scroll" style="padding:0 24px;">
        ...
        <p style="margin:0 0 28px; font-size:14px; font-weight:300; line-height:1.7; color:#5B5449;">Questions about the contest, a problem with an upload, or an idea for the program — it all lands with the ArtUP team.</p>
        ...
        <div id="cats" style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:26px;">
          ${CATEGORIES.map((c) => `<span data-cat="${c}" style="height:36px; padding:0 15px; border-radius:18px; display:flex; align-items:center; font-size:12px; cursor:pointer; ${c === category ? 'background:#1B1916; color:#FBF8F2; font-weight:400;' : 'border:1px solid rgba(27,25,22,.18); font-weight:300; color:#4A443A;'}">${c}</span>`).join('')}
        </div>
        ...
        <div style="border-radius:16px; background:#FFFFFF; border:1px solid rgba(27,25,22,.12); padding:16px 18px 12px; margin-bottom:10px;">
          ...
          <div style="display:flex; justify-content:flex-end; border-top:1px solid rgba(27,25,22,.08); padding-top:10px;">
            <span id="fb-count" style="font-size:11.5px; font-weight:300; color:#8C8375;">0 / 1000</span>
          </div>
        </div>
        ...
        <div style="border-radius:16px; background:#F2ECE0; padding:18px 20px; margin-bottom:26px;">
          ...
          <p style="margin:0; font-size:14px; font-weight:300; color:#1B1916;">${esc(p.name)} · ${esc(p.email)}</p>
        </div>
      </div>
  ...
  root.innerHTML = `
    <div class="screen">
      <div class="scroll" style="display:flex; flex-direction:column; align-items:center; justify-content:center; text-align:center; padding:0 32px;">
        ...
        <p style="margin:0; max-width:280px; font-size:14px; font-weight:300; line-height:1.7; color:#5B5449;">Your message is with the ArtUP team. We reply by email when there's something to share.</p>
      </div>
